...
from posts.utils import (
    get_top_tag_after_translation_only_label,
...
class TagInferenceTestCase(TestCase):
    def assert_helper(self, text, expected_tag):
        ...
        matching_tag = get_top_tag_after_translation_only_label(
            possible_tags=possible_tags, translated_description=translated_description
        )
    ...
    def test_tag_inference(self):
        test_cases = {
            ...
            # "네, 예전부터 좋아하던 곳이예요. 부채살 스테이크랑 매쉬드 포테이토랑 같이 먹으면 조화롭기가 세상 맛있답니다😋  가족끼리 가서 먹기도 좋을것 같아": "for Family Gatherin",
            ...
            "사람 진짜 엄청 많습니다! 앉는곳이 없다시피 해서 시간 맞춰서 가세용 9시에 열리는 캐치테이블 웨이팅 등록후 출발하세요 1시간 50분 기다린후 들어갔습니다 포장으로 했는데 다음에는 먹고 가고 싶네용 넘무 친절하시고 베이글도 먹음직스럽습니다": "trending, hot, instagram",
            "음식이 하나하나 정갈하고 고급스러워서 부모님 모시고 오기에 참 좋은 장소같습니다. 상대적으로 좀 비싸고 워낙 외진 곳에 있어 강추하기는 좀 그렇구요.": "Luxurious, Expensive",
            "갓김치 너무 맛있어용, 마제소바도 맛있게 잘 먹었습니다ㅎㅎ, 혼밥하기에도 넘 좋아요!": "Alone",
            "곰탕 고기가 진짜 푸짐해요 5천원이라고 믿기지 않는 맛과 양..": "Cost-Effective",
            "헌팅포타답게 시끌시끌해요 20대 중후반보다는 초반 친구들이 많이 와서 그런지 엄청 활기차더라구요": "Noisy",
            "소개팅 처음 했을 때도 여기 왔는데 이제 남자친구랑 같이 왔어요 ㅎㅎ 분위기가 너무 좋고 파스타도 참 맛있습니다": "for date, couple",
            "사장님 겁나 친절함 심지어 단골 되면 후식으로 요구르트도 주심.. 왜 안 와?": "excellent staff service",
            "미리 예약해서 룸으로 들어갔는데 음악도 잔잔하고 서빙도 격조 있게 해주셔서 좋았어요. 요즘 고깃집들 들어가면 이모 부르는 소리에 너무 시끄러운데 여긴 안 그래서 좋더라구요": "Quiet, Calm",
            "응 밀크티 존맛탱 안 먹으면 상종 안 해요": "Noisy",
            "인스타에서 하도 뜨길래 소금빵 사러 왔는데 줄이 너무 길어서 당황.. 텐션 다 떨어져서 한 입 먹었는데 너무 맛있어서 바로 회복됐어요": "trending, hot, instagram",
            "도미지리가 진짜 맛있어요 ㅠㅠ 비싸긴 한데 비싼 값을 합니다 회도 진짜 맛있어요! 추천": "Luxurious, Expensive",
            "부모님 생일 기념으로 당일 예약 케이크 했는데 너무 귀엽게 만들어주셨어요! 가격도 합리적이에요": "Cost-Effective",
            "버거킹은 역시 관악구청점이 제일 맛있어요 맨날 혼자 오는데 눈치 안 보고 먹을 수 있어서 좋네요 다음에는 친구 사귀어서 데리고 올게요": "Alone",
        }
